import string

hex_data = ""
ciphertext = bytes.fromhex(hex_data)
prefix = b"CSC{"

print(f"{'Offset':<10} | {'Potential Key':<15}")
print("-" * 30)

for i in range(len(ciphertext) - 4):
    # XOR 4 bytes of ciphertext with 'CSC{' to see what the key would be
    key_guess = bytes([ciphertext[i+j] ^ prefix[j] for j in range(4)])
    
    # Check if the 4 bytes we found are all printable ASCII (letters/numbers)
    if all(b in range(32, 127) for b in key_guess):
        try:
            print(f"{i:<10} | {key_guess.decode()}")
        except:
            pass